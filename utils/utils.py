...
def generate_qrcode(
    ...
    def calc_text_size(word_width, word_height) -> tuple:
        assert text_direction in ('h', 'v'), f'ValueError, Get Wrong Direction {text_direction}'
    ...
    qr.add_data(qr_info,); qr.make(fit = True)
    ...
    if add_info:
        ...
        match text_position:
            ...
            case 'top':
                gene_image.paste(qr_image, ((gene_width - qr_image.size[0])//2, text_size[1]))
            case 'bottom':
        ...
        for index, tiny in enumerate(add_info, 0):
            ...
            drawer.text(text_location, format_tiny, font = font, fill = 'black', align = 'center')
    else:
...
class Dot_Dict(dict):
    ...
    def update(self, key: str, value: dict | str) -> bool:
        *wrapper, key = key.split('.'); father = None
